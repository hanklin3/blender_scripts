...
parser.add_argument('--views', type=int, default=2, help='number of views to be rendered')
...
def setup_nodes():
	...
	links.new(render_layers.outputs['DiffDir'], diffuse_file_output.inputs[0]) # DiffCol / DiffDir

	specular_file_output = tree.nodes.new(type="CompositorNodeOutputFile")
	specular_file_output.label = 'Specular Output'
	links.new(render_layers.outputs['GlossDir'], specular_file_output.inputs[0]) # GlossCol / GlossDir
...
def setup_camera():
	...
	# scene.render.alpha_mode = 'TRANSPARENT'  TODO , 2.82?
